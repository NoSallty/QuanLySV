...
/**
 * Servlet implementation class SuaMHController
 */
@WebServlet("/editSubject")
public class SuaMHController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public SuaMHController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		String mahp = request.getParameter("smaHP");
		MonHocDao s = new MonHocDao();
		
		MonHoc o = s.getSubjectObjectByMaHP(mahp);
		
		request.setAttribute("ed", o);
		request.getRequestDispatcher("TrangSuaMonHoc.jsp").forward(request, response);
	}
